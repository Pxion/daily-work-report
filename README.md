...
### 2024.6.6
报错原因为无法找到msg和srv文件，可我明明是有的，最终我把msg和srv文件夹从src文件夹里提取到外面后就编译成功了，ROS2接口常用CLI（命令行）命令。

### 2024.8.5
前期由于双创活动及三下乡社会实践等各种任务缠身，没有明显的进度，只是把之前的内容重新大致看了一下，发现忘了挺多的，但是接着先学吧，后期可能用到的时候再回顾，在应用中学习可能效果会比较好。
自定义接口rclcpp实战：创建功能包及节点
